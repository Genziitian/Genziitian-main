...
const qualifierPlans: Record<string, { price: number }> = {
  basic: { price: 449 },
  advanced: { price: 649 },
  champion: { price: 999 },
};
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (qualifierPlan) {
      const plan = qualifierPlans[String(qualifierPlan).toLowerCase()];
      ...
      const expectedEnrollmentId = `${qualifierBaseCourseId}-${String(qualifierPlan).toLowerCase()}`;
      if (courseIds.length !== 1 || courseIds[0] !== expectedEnrollmentId) {
        return res.status(400).json({ error: 'Invalid qualifier enrollment ID' });
      }

      totalOriginalPrice = plan.price;
