...
const qualifierPlanMap = {
  basic: { label: 'Basic', price: 449 },
  advanced: { label: 'Advanced', price: 649 },
  champion: { label: 'Champion', price: 999 },
} as const;

type QualifierPlanId = keyof typeof qualifierPlanMap;
...
export default function CourseSelection() {
  ...
  const rawQualifierPlan = searchParams.get('plan');
  const qualifierPlan = rawQualifierPlan && rawQualifierPlan in qualifierPlanMap
    ? rawQualifierPlan as QualifierPlanId
    : null;
  const qualifierPlanDetails = qualifierPlan ? qualifierPlanMap[qualifierPlan] : null;
  ...
  const fetchCourse = async () => {
    try {
      ...
      if (data.isQualifierCourse && qualifierPlan) {
          setSelectedCourses([`${data.id}-${qualifierPlan}`]);
  ...
  const calculateTotal = () => {
    if (!course) return 0;
    if (course.isQualifierCourse && qualifierPlanDetails) return qualifierPlanDetails.price;
  ...
  const handlePayment = async () => {
    ...
    try {
      const orderData = await apiService.createOrder({
        ...
        qualifierPlan: course?.isQualifierCourse ? qualifierPlan || undefined : undefined,
        qualifierBaseCourseId: course?.isQualifierCourse ? course.id : undefined,
      });
      ...
      const options = {
        ...
        currency: "INR",
        name: "GenZ IITIAN",
        description: course.isQualifierCourse && qualifierPlanDetails
          ? `${course.name} - ${qualifierPlanDetails.label}`
          : course.isBundle 
          ? course.bundleCourses.filter((bc: any) => selectedCourses.includes(bc.courseId)).map((bc: any) => bc.courseName).join(', ').substring(0, 255)
          : course.name,
        notes: {
          courses: course.isQualifierCourse && qualifierPlanDetails
            ? `${course.name} - ${qualifierPlanDetails.label}`
            : course.isBundle 
            ? course.bundleCourses.filter((bc: any) => selectedCourses.includes(bc.courseId)).map((bc: any) => bc.courseName).join(', ')
            : course.name,
        ...
        handler: async (response: any) => {
          ...
          try {
            ...
            const courseTitle = course.isQualifierCourse && qualifierPlanDetails
              ? `${course.name} - ${qualifierPlanDetails.label}`
              : course.isBundle 
              ? course.bundleCourses.filter((bc: any) => selectedCourses.includes(bc.courseId)).map((bc: any) => bc.courseName).join(', ')
              : course.name;
                    ...
                    {!course.isBundle || course.isFixedBundle ? (
                        <div className="p-3.5 bg-blue-50 border-[2px] border-[#0b1120] rounded-xl flex items-center justify-between">
                            <div className="flex items-center gap-2.5">
                                ...
                                <span className="font-black text-base text-[#0b1120]">
                                  {course.isQualifierCourse && qualifierPlanDetails ? `${course.name} - ${qualifierPlanDetails.label}` : course.name}
                                </span>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex items-end gap-2">
                                    <div className="text-2xl font-black text-[#0b1120]">
                                      ₹{Math.max(
                                        (() => {
                                          const rawTotal = course?.isQualifierCourse && qualifierPlanDetails
                                            ? qualifierPlanDetails.price
                                            : course?.isBundle 
                                            ? course.bundleCourses
                                                .filter((bc: SubCourse) => selectedCourses.includes(bc.courseId))
                                                .reduce((sum: number, bc: SubCourse) => sum + bc.price, 0)
