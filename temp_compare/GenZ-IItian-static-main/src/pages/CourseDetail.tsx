...
import { getCheckoutPath, getQualifierCheckoutPath } from '../utils/courseRouting';
...
const qualifierPlans = [
  ...
  },
  {
    ...
    price: 649,
    badge: 'Most Popular',
    accent: 'border-[#4f46e5] bg-[#eef2ff]',
    button: 'bg-[#4f46e5] hover:bg-[#4338ca]',
    features: [
      'Subjects: CT, Stats 1, Maths 1, Eng 1',
  ...
  },
  {
    ...
    guarantee: 'Full Refund if Not Qualified!',
...
export default function CourseDetail() {
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      ...
      {course.isQualifierCourse && (
        <section className="bg-[#f8fafc] px-4 sm:px-6 py-12 lg:py-14 border-b border-gray-100">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-4xl font-black text-[#0b1120] mb-3">
                IIT Madras BS Qualifier Plans
              </h2>
              <p className="text-gray-600 font-bold max-w-2xl mx-auto">
                Choose the level of support you need for CT, Stats 1, Maths 1, and English 1.
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8 items-stretch">
              {qualifierPlans.map((plan) => (
                <div
                  key={plan.id}
                  className={`relative flex min-h-[440px] flex-col rounded-2xl border-[3px] ${plan.accent} bg-white p-6 lg:p-8 shadow-[0_14px_32px_rgba(15,23,42,0.10)]`}
                >
                  {plan.badge && (
                    <div className={`absolute right-0 top-0 rounded-bl-xl rounded-tr-xl px-4 py-2 text-xs font-black ${plan.id === 'champion' ? 'bg-[#22c55e] text-white' : 'bg-[#ffd21f] text-[#0b1120]'}`}>
                      {plan.badge}
                    </div>
                  )}

                  <h3 className="text-xl font-black text-[#0b1120] mb-4">{plan.title}</h3>
                  <div className="text-4xl font-black text-[#4f46e5] mb-4">₹{plan.price}</div>
                  {plan.guarantee && (
                    <p className="text-sm font-black text-[#16a34a] mb-5">{plan.guarantee}</p>
                  )}

                  <ul className="space-y-3 mb-8 text-sm text-[#0b1120]">
                    {plan.features.map((feature, index) => (
                      <li key={feature} className={index === 0 ? 'font-bold' : 'font-medium'}>
                        {feature}
                      </li>
                    ))}
                  </ul>
                  ...
                  {plan.terms && (
                    <p className="text-xs italic text-gray-500 mb-6">{plan.terms}</p>
                  )}

                  <Link
                    to={getQualifierCheckoutPath({ id: String(course.id), name: course.name }, plan.id)}
                    className={`mt-auto w-full rounded-lg px-5 py-4 text-center font-black text-white transition-colors ${plan.button}`}
                  >
                    Join Now
                  </Link>
                </div>
