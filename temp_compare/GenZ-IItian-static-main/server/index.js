...
app.use('/blog-admin', express.static(path.join(__dirname, '..', 'admin')));
...
app.post('/api/create-order', async (req, res) => {
    ...
    const { email, courseIds, bundleId, discountCode, referralCode, coinsToApply, qualifierPlan, qualifierBaseCourseId } = req.body;
    ...
    try {
        ...
        // --- QUALIFIER PLAN FLOW ---
        if (qualifierPlan) {
            const planKey = String(qualifierPlan).toLowerCase();
            const plan = qualifierPlans[planKey];
            if (!plan || !qualifierBaseCourseId) {
                return res.status(400).json({ error: 'Invalid qualifier plan' });
            }

            const { data: qualifierCourse, error: qualifierError } = await supabase
                .from('courses')
                .select('id, isQualifierCourse')
                .eq('id', qualifierBaseCourseId)
                .eq('isQualifierCourse', true)
                .single();

            if (qualifierError || !qualifierCourse) {
                return res.status(400).json({ error: 'Invalid qualifier course' });
            }

            const expectedEnrollmentId = `${qualifierBaseCourseId}-${planKey}`;
            if (!Array.isArray(courseIds) || courseIds.length !== 1 || courseIds[0] !== expectedEnrollmentId) {
                return res.status(400).json({ error: 'Invalid qualifier enrollment ID' });
            }

            totalAmount = plan.price;
            originalAmount = plan.price;
            orderNotes = { courses: `${qualifierCourse.id} - ${planKey}`, user_email: email };
        } else if (bundleId) {
